...
def roi(image):
    ...
    top_right = [0, height * 14 / 30]
    top_left = [width, height * 14 / 30]
...
def filtering_lines(image, lines):
    ...
    for x1, y1, x2, y2 in lines[:, 0]:
        ...
        elif -2 <= slope <= -0.4 and intercept >= -300:
    ...
    if lines is not None:
        ...
        if len(lines) == 2:
            ...
            if lines[1] is not None:
                ...
                # turn prediction
                if m > 0.8:
                    cv2.putText(mask, 'Going Straight', position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3,
                                cv2.LINE_AA)
                elif 0 <= m <= 0.8:
                    cv2.putText(mask, 'Turning Right', position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3,
                                cv2.LINE_AA)
                elif m <= 0:
                    cv2.putText(mask, 'Turning Left', position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3,
                                cv2.LINE_AA)
                if flag:
                    plot_image(mask, "mask")
                return mask
...
files = glob.glob('C:/Users/lenovo/PycharmProjects/ENPM673Proj2/data/*.png')

# defining size using dimensions of the given frame
size = (1392, 512)

# creating output video file
out = cv2.VideoWriter('FinalOutput21.avi', cv2.VideoWriter_fourcc(*'DIVX'), 5, size)
...
for i in range(len(files)):
    ...
    if frame is not None:
        ...
        Hough_lines_image = Hough_line(detected_edges_canny, 0.9, np.pi / 180, 25, 50, 2500)
